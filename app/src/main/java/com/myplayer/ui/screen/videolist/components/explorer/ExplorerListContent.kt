package com.myplayer.ui.screens.videolist.components.explorer
...
import com.myplayer.ui.screen.videolist.components.video.VideoGridItem
import com.myplayer.ui.screen.videolist.components.video.VideoListItem
import com.myplayer.ui.screen.videolist.components.folder.FolderGridItem
import com.myplayer.ui.screen.videolist.components.folder.FolderListItem
...
fun ExplorerListContent(
    ...
    val mostRecentlyPlayedUri = remember(items, historyMap) {
        items.filterIsInstance<ExplorerItem.VideoItem>()
             .mapNotNull { item -> historyMap[item.video.uri]?.let { h -> item.video.uri to h.lastPlayedAt } }
             .filter { it.second > 0L }
